...
flightsCsvPath <- "../data/flights.csv"

# Create a local R dataframe
flights_df <- read.csv(flightsCsvPath, header = TRUE)
flights_df$date <- as.Date(flights_df$date)

## Filter flights whose destination is San Francisco 
# and write to a local data frame
...
flightsDF <- read.df(sqlContext, flightsCsvPath, 
