...
def getSimilarity(a,b):
    mean_a = pd.Series.mean(a)
    mean_b = pd.Series.mean(b)
    std_a = pd.Series.std(a)
    std_b = pd.Series.std(b)
    s = 0
    num = 0
    for j in range(len(a)):
        num += ((a[j]-mean_a)*(b[j]-mean_b))
    s = num/(len(a)*std_a*std_b)
    return s
...
def computeDelta(wt, X, Xi):
    ...
    X : A row of Panda Dataframe for train2
        Corresponds to (x, y) in Equation 6.
    Xi : Panda Dataframe (full dataset train1)
    ...
    #For the dataset train2
    mask = X.index.isin(['Yi'])
    x = X.loc[~mask]
    y = X.loc[mask]#This wont be required.

    num = 0
    den = 0
    for i in xrange(0,len(Xi.index)):
        row = Xi.iloc[i]
        mask = row.index.isin(['Yi'])
        xi = row.loc[~mask]
        yi = row.loc[mask]
        sim_val = getSimilarity(x,xi)
        num += (yi*math.exp(wt*sim_val))
        den += math.exp(wt*sim_val)
    e_emp = num/den
    return e_emp
